...
(ref{eq:2:auto}) с
начальным условием $x(t_0)=x_0$. Пусть $f(x_0)\ne 0$. В этом случае решение задаётся явной формулой (она называется *формулой Баррой*). Мы обсудим несколько способов её вывода и интерпретации.
...
мгновенно сводящиеся интегрированию (см. раздел ref{sec:1:onlytime}). Мы
...
\end{align}
!et
Это может показаться некоторой магией — особенно загадочно выглядит уравнение (ref{eq:2:magic}). Чуть позже мы дадим определение *дифференциальной 1-формы*, с помощью которого можно придать этим формулам аккуратный смысл, а пока обратим внимание, что уравнение (ref{eq:2:magic}) очень похоже на уравнение (ref{eq:2:Delta_t}). В целом, эта формальная запись фактически повторяет наш вывод в разделе ref{sec:2:mech}.
