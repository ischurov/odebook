...
\end{equation}
!et
где $D(f)$ — область определения функции $f$: это может быть вся числовая ось, луч, отрезок, интервал или полуинтервал. См. также замечание ref{rem:1:domain}

!bu-remark label=rem:1:domain
Мы будем предполагать, что областью определения решения
